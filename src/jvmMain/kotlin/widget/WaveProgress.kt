...
fun WaveProgress(modifier: Modifier) {
    ...
    val yList = Array(count) {
        topLineHeight - abs(sin(it * unit)) * range * 10
    }
    ...
    var bottomPadding by remember { mutableStateOf((30..50).random()) }
    var topPadding by remember { mutableStateOf((-20..40).random()) }
    var horizonOffset by remember { mutableStateOf((5..40).random()) }
    offsetList.clear()
    sList.forEach {
        it.update()
    ...
    LaunchedEffect(sList.size) {
        flow {
            ...
            if (top) {
                pointCount++
                sList.add(Square(horizonOffset+width+width/5 , height / 2 - topPadding))
                if (pointCount == 2) {
                    top = false
                    pointCount = 0
                ...
                pointCount++
                sList.add(Square(horizonOffset+width+width/5 , topLineHeight - bottomPadding))
                if (pointCount == 2) {
                    top = true
                    pointCount = 0
    ...
    Box(modifier = modifier) {
        ...
        Canvas(modifier = modifier) {
            ...
            drawLine(
                color = lineColor,
                start = Offset(0f, topLineHeight),
                end = Offset(width, topLineHeight),
                strokeWidth = 3f
            )
            drawLine(
                color = lineColor,
                start = Offset(0f, secondHeight),
                end = Offset(width, secondHeight),
                strokeWidth = 3f
            )
            drawLine(
                color = lineColor,
            ...
            drawLine(
                color = lineColor,
                start = Offset(0f, fourHeight),
                end = Offset(width, fourHeight),
                strokeWidth = 3f
            )
            rightPosList.forEach {
                drawLine(color = lineColor, start = Offset(0f, topLineHeight), end = it, strokeWidth = 3f)
            }
            leftPosList.forEach {
                drawLine(color = lineColor, start = Offset(width, topLineHeight), end = it, strokeWidth = 3f)
            }
